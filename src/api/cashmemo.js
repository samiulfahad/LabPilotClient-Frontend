...
const cashmemoService = {
  ...
  getIpdDeletedPatients: ({ startDate, endDate }) => {
    const params = new URLSearchParams({ startDate, endDate });
    return api.get(`/cashmemo/ipd-deleted-patients?${params}`);
  },
  getOutdoorDeletedInvoices: ({ startDate, endDate }) => {
    const params = new URLSearchParams({ startDate, endDate });
    return api.get(`/cashmemo/outdoor-deleted-invoices?${params}`);
  },
