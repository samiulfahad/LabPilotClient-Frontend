...
// chronological entry list for display inside a staff's expanded panel.
...
const EMPTY_DATA = {
  staff: [],
  totals: { totalDiscount: 0, opdDiscount: 0, ipdDiscount: 0 },
};
...
// Small pill showing a source's (OPD/IPD) share of a discount total.
...
const StaffEntry = ({ member: m, rank, isHospital }) => {
  ...
  const entries = mergeEntries(m.invoices, m.patients);
    ...
    <section className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 px-4 py-6 font-noto">
      ...
      <div className="max-w-2xl mx-auto">
              ...
              <div>
                ...
                {!isStaff && (
                  <div className="flex flex-wrap divide-x divide-[#E3E0D6] mt-3">
                    ...
                    {isHospital && (
                      <>
                        <HeaderStat label="OPD" value={`৳${fmt(d.totals.opdDiscount)}`} />
                        <HeaderStat label="IPD" value={`৳${fmt(d.totals.ipdDiscount)}`} accent={SEAL_BLUE} />
                      </>
                    )}
                  </div>
                )}
        ...
        <p className="font-['IBM_Plex_Mono'] text-center text-xs text-[#A8ACA3] mt-4 pb-6 no-print font-noto">
          শুধুমাত্র সক্রিয় (ডিলিট না হওয়া) এডমিশনের ডিসকাউন্ট অন্তর্ভুক্ত
        </p>
      </div>
    </section>
  );
