...
const Test = ({ input, onConfigure, onDelete }) => {
  const { name, price, isOnline, formats } = input;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-5">
        ...
        <div className="flex items-start gap-4 flex-1 min-w-0">
          ...
          <div className="relative flex-shrink-0">
            <div className="w-12 h-12 rounded-2xl flex items-center justify-center bg-gradient-to-br from-teal-50 to-cyan-50 text-teal-700 shadow-sm group-hover:shadow transition-all duration-300">
              <FlaskConical className="w-5 h-5" />
            </div>
            <div
              className={`absolute -bottom-0.5 -right-0.5 w-4 h-4 rounded-full border-2 border-white shadow-sm flex items-center justify-center ${isOnline ? "bg-emerald-500" : "bg-amber-500"}`}
            >
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span
                className={`px-2.5 py-0.5 rounded-full text-xs font-medium inline-flex items-center gap-1.5 ${isOnline ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
              >
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-gray-50 rounded-full text-xs">
                ...
                <span className="font-medium text-gray-700">{price ? `৳${price}` : "Price NOT set"}</span>
              </div>
              {formats?.length > 0 && (
        ...
        <div className="flex gap-2 flex-wrap sm:flex-nowrap">
          <button
            onClick={onConfigure}
            className="flex-1 sm:flex-initial px-4 py-2 rounded-xl text-sm font-medium inline-flex items-center justify-center gap-2
