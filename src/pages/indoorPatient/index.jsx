...
import { BedDouble, UserPlus, Users, Search } from "lucide-react";
        ...
        <div className="grid grid-cols-2 gap-3">
          {ipdNav.map((item) => {
            ...
            return (
              ...
              >
                {/* Top accent bar */}
                <div className={`absolute top-0 left-0 right-0 h-[3px] bg-gradient-to-r ${c.bar} scale-x-0 group-hover:scale-x-100 transition-transform duration-200 origin-left`} />

                {/* Icon */}
                <div className={`w-12 h-12 rounded-[13px] border border-slate-200 bg-slate-50 flex items-center justify-center transition-all duration-200 ${c.iconBox}`}>
                  <Icon size={20} className={`transition-colors duration-200 text-slate-400 ${c.icon}`} />
                </div>

                {/* Text */}
                <div className="text-center space-y-1">
                  <p className={`text-[13px] font-bold text-slate-700 tracking-tight transition-colors duration-200 ${c.label}`}>
                    {item.label}
                  </p>
