...
const resolveReferrerDiscount = (referredBy, hasReferrerDiscount, referrerDiscount, initial) => {
  ...
  if (referredBy.commissionType === "percentage" && referrerDiscount > 0)
    return parseFloat(((initial * referrerDiscount) / 100).toFixed(2));
  if (referredBy.commissionType === "fixed") return parseFloat(referrerDiscount) || 0;
  return 0;
};

const computePricing = (formData) => {
  const initial = formData.selectedTests.reduce((sum, t) => sum + (t.price || 0), 0);
  const referrerDiscount = resolveReferrerDiscount(
    formData.referredBy,
    formData.hasReferrerDiscount,
    formData.referrerDiscount,
    initial,
  );
  const labAdjustment = formData.hasLabAdjustment ? formData.labAdjustmentAmount || 0 : 0;
  const final = Math.max(0, initial - referrerDiscount - labAdjustment);
  return { initial, referrerDiscount, labAdjustment, final };
};
...
const InvoiceSummary = ({ formData, pricing, onConfirm, onClose }) => {
  const { patient, referredBy, selectedTests, paidAmount, hasReferrerDiscount, referrerDiscount, hasLabAdjustment } =
    formData;
  const { initial, referrerDiscount: discountAmount, labAdjustment, final } = pricing;
  const dueAmount = Math.max(0, final - (paidAmount || 0));
        ...
        <div className="bg-gray-50 rounded-xl p-5">
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-gray-900">{formatCurrency(initial)}</span>
            </div>
            {hasReferrerDiscount && discountAmount > 0 && (
              <div className="flex justify-between">
                <span className="text-gray-600">
                  Referrer Discount {referredBy?.commissionType === "percentage" ? `(${referrerDiscount}%)` : "(Fixed)"}
                </span>
                <span className="text-red-600">- {formatCurrency(discountAmount)}</span>
              </div>
            )}
            {hasLabAdjustment && labAdjustment > 0 && (
              <div className="flex justify-between">
                <span className="text-gray-600">Lab Adjustment</span>
                <span className="text-red-600">- {formatCurrency(labAdjustment)}</span>
              </div>
            )}
            <div className="flex justify-between pt-3 border-t-2 border-gray-200 text-base">
              <span className="font-semibold text-gray-900">Total Amount</span>
              <span className="text-xl font-bold text-blue-600">{formatCurrency(final)}</span>
            </div>
...
const InvoiceForm = ({
  ...
  const dueAmount = Math.max(0, final - (paidAmount || 0));
  const isFullyPaid = paidAmount >= final && final > 0;
  ...
  // Derived display value for referrer input
  const referrerInputValue =
      ...
      <SectionCard icon={DollarSign} title="Pricing & Adjustments">
        <div className="space-y-5">
          <div className="p-4 bg-gray-50 rounded-lg flex items-center justify-between">
            ...
            <span className="text-xl font-semibold text-gray-900">{formatCurrency(initial)}</span>
          </div>
              ...
              {hasReferrerDiscount && (
                <div className="ml-6 p-4 bg-blue-50 rounded-lg border border-blue-100">
                  ...
                  <div className="mt-3 p-3 bg-white rounded-lg border border-blue-200 flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-blue-600">{formatCurrency(initial - discountAmount)}</span>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="p-5 bg-blue-600 rounded-lg flex items-center justify-between">
            ...
            <span className="text-2xl font-bold text-white">{formatCurrency(final)}</span>
          </div>
...
const CreateInvoice = () => {
  ...
  // pricing is derived — never stored in state
  const pricing = useMemo(
    () => computePricing(formData),
    [
  ...
  const handlePreview = (e) => {
    ...
    if (selectedTests.length === 0) return setPopup({ type: "error", message: "Please select at least one test" });
  ...
  const handleConfirm = async () => {
    try {
      setLoading(true);
      const { patient, referredBy, selectedTests, paidAmount } = formData;
      const { initial, referrerDiscount, labAdjustment, final } = pricing;
      ...
      const invoiceData = {
        ...
        pricing: { initial, referrerDiscount, labAdjustment, final },
